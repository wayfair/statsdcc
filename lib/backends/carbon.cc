...
void Carbon::flush_stats(const Ledger& ledger, int flusher_id) {
  ...
  for (auto counter_itr = ledger.counters.cbegin();
    ...
    std::string metric_name = this->prefix + ::config->prefix_counter + this->process_name(key);
  ...
  for (auto timer_itr = ledger.timer_data.cbegin();
    ...
    std::string metric_name = this->prefix + ::config->prefix_timer + this->process_name(key);
  ...
  for (auto gauge_itr = ledger.gauges.cbegin();
    ...
    std::string metric_name = this->prefix + ::config->prefix_gauge + this->process_name(key);
  ...
  for (auto set_itr = ledger.sets.cbegin();
    ...
    std::string metric_name = this->prefix + ::config->prefix_set + this->process_name(key);
